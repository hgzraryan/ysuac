...
session_start();
	...
	// Uploading folder
	$folder = '../../images/gallery_category/';
	if (!is_dir($folder))
		mkdir($folder);
	// If specifics folder 
	...
	if (preg_match('/image/i', $_FILES['xfile']['type'])) {
		$image_name=sha1(@microtime() . '-' . $_FILES['xfile']['name']) . '.jpg';
		$filename = $_POST['value'] ? $_POST['value'] :
				$folder . $image_name;
				$_SESSION['cat_image']=$image_name;
	...
	// File path
	$path = str_replace('upload_gallery.php', '', $_SERVER['SCRIPT_NAME']);
